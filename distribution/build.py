...
def generate_config_labels(version: str) -> str:
    """Generate OCI LABEL instruction with base64-encoded config."""
    ...
    labels = [
        ...
        ("org.opencontainers.image.title", "OGX - rh distribution"),
        ("org.opencontainers.image.version", version),
    ]

    first = f'{labels[0][0]}="{labels[0][1]}"'
...
def generate_containerfile(dependencies, llama_stack_install):
    ...
    containerfile_content = containerfile_content.replace(
        ...
        llama_stack_install if llama_stack_install else "",
    )
    containerfile_content = containerfile_content.replace(
        "{config_labels}", generate_config_labels(LLAMA_STACK_VERSION)
    )
...
def generate_konflux_dockerfile():
    """Generate Dockerfile.konflux from template, if the template exists."""
    ...
    content = warning + template_content
    content = content.replace("{config_labels}", generate_config_labels("${LLAMA_STACK_VERSION}"))
