#!/usr/bin/env bash
# Verify that OGX config labels are properly embedded in an image
set -euo pipefail
...
for config in "${CONFIGS[@]}"; do
    ...
    source_file="distribution/${config}"
    if [ ! -f "$source_file" ]; then
        echo "   Source file not found: $source_file (skipping comparison)"
        rm -f "$decoded_file"
        continue
    fi
